...
3. if any canonical output changed: a new edition has been declared in
   ...
   regenerated in the same MR that changed it. **A determinism change
...
all platforms reproducibly, generates SBOMs, writes `checksums.txt`,
signs it with keyless cosign (this project's CI OIDC identity — there
is no long-lived signing key to rotate or leak), and publishes the
GitLab release. The `release-vsix` job packages the editor extension
and publishes to the marketplace when a token is configured.
...
1. **Stop the bleeding**: delete the GitLab release so the permalink
   `releases/permalink/latest` stops serving it
   (Project → Releases → delete; the tag may stay for forensics).
